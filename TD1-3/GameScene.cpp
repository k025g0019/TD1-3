...
void SceneManager::Update(char* keys, char* preKeys)
{
	...
	case SceneType::STAGESELECT:
	{
		...
		// --- 1. キーボード ---
		if ((keys[DIK_LEFT] && !preKeys[DIK_LEFT]) || (keys[DIK_A] && !preKeys[DIK_A]))
		{
			moveLeft = true;
		}
		if ((keys[DIK_RIGHT] && !preKeys[DIK_RIGHT]) || (keys[DIK_D] && !preKeys[DIK_D]))
		{
			moveRight = true;
		}
